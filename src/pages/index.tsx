    ...
    if (account && account.address) {
      try {
        ...
        if (service_aggregator) {
          ...
          const service_infos_map_handle: string =
            service_aggregator.data.services_map.handle;
          // console.log(addresses);
          // console.log(addr_infos_map_handle);
          const res: Array<any> = [];
          for (let i = 0; i < names.length; i++) {
            const table_item = await client.aptosClient.getTableItem(
              service_infos_map_handle,
              {
